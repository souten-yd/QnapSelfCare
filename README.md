...
0.3.1以降からは上記ボタンで0.3.4へ更新できます。0.3.0以前から今回版への切替だけはApp CenterでQPKGを手動適用してください。以後は「設定・バックアップ → SelfCareの更新 → 更新を確認 → vX.Y.Zに更新する」で、ブラウザへQPKGをダウンロードせず更新できます。
...
```sh
./selfcare-update check --current-version 0.3.4 --arch x86_64
./selfcare-update download --current-version 0.3.4 --arch x86_64 --dest /path/to/private/staging
```
