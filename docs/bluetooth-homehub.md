...
SelfCare 0.3.4以降では「設定・バックアップ → 共通Bluetoothのセットアップ → 共通Bluetooth構成へ更新」から標準構成を移行できます。[移行機能の詳細](homehub-migration.md)を参照してください。HomeHub側の従来の更新ボタンだけではComposeサービスは追加されません。
