...
- HEM-6232T memory layout and classic authentication/control-response flow are based on hass-omron `omron_ble/device_catalog.py`, `record_parsers.py`, `unlock.py`, `session.py`, and `memory_protocol.py`, Copyright (c) 2025 eigger (MIT): https://github.com/eigger/hass-omron/tree/master/custom_components/omron/omron_ble . The MIT notice is included in `licenses/hass-omron-MIT.txt`.
