...
cp "$root/storage.py" "$root/collectors.py" "$root/ble_protocol.py" "$root/ble_worker.py" "$root/ble_bridge.py" "$root/self_update.py" "$root/requirements-ble.txt" "$root/THIRD_PARTY.md" "$root/README.md" "$work/shared/"
